...
def create_single_metric_plots(summary_file='summary_no_bootstrap.csv', output_dir='.'):
    """Plot metrics from a single summary file"""
    ...
    for metric in metrics:
        ...
        # Plot data
        ax = sns.lineplot(data=df_summary, x='range_idx', y=metric, marker='o', color='#1f77b4')
        ...
        # Save plot
        plt.savefig(os.path.join(output_dir, f'single_{metric}.png'), dpi=300)
        plt.close()
    ...
    for i, metric in enumerate(metrics):
        plt.subplot(2, 3, i+1)
        sns.lineplot(data=df_summary, x='range_idx', y=metric, marker='o', color='#1f77b4')
        plt.title(f"{metric.upper()} by Weight Range")
        plt.xlabel("Weight Range Index")
        plt.ylabel(metric.upper())
        plt.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'single_comparison.png'), dpi=300)
    plt.close()
    
    print("Single file plots created successfully.")
